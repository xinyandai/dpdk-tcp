...
static inline void xy_mbuf_list_add_head(xy_mbuf_list *head,
                                         struct rte_mbuf *mbuf) {
  xy_mbuf_list *new_node = rte_pktmbuf_mtod(mbuf, xy_mbuf_list *);
  new_node->mbuf = mbuf;

  xy_list_add_head((xy_list_node *)head, (xy_list_node *)new_node);
}
...
static inline void xy_mbuf_list_del(struct rte_mbuf *mbuf) {
  xy_mbuf_list *del_node = rte_pktmbuf_mtod(mbuf, xy_mbuf_list *);
  xy_assert(del_node->mbuf == mbuf);

  xy_list_del((xy_list_node*) del_node);
}

static inline int xy_mbuf_list_is_empty(xy_mbuf_list *head) {
  return ((xy_list_node*) head)->next == (xy_list_node*) head;
}
...
static inline struct rte_mbuf * xy_mbuf_list_take_tail(xy_mbuf_list *head) {
  xy_list_node * entry = xy_list_take_tail((xy_list_node *)head);
  return ((xy_mbuf_list*) entry)->mbuf;
}
