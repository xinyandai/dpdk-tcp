...
inline int arp_recv(struct rte_mbuf *m_buf, struct rte_ether_hdr *eh,
                    int len) {
  auto *ah =
      (struct rte_arp_hdr *)((unsigned char *)eh + RTE_ETHER_HDR_LEN);

  if (len < (int)(sizeof(struct rte_ether_hdr) +
                  sizeof(struct rte_arp_hdr))) {
    return 0;
  }

  if (rte_cpu_to_be_16(ah->arp_opcode) !=
      RTE_ARP_OP_REQUEST) {  // ARP request
    return 0;
  }

  if (xy_this_ip == ah->arp_data.arp_tip) {
    rte_memcpy((unsigned char *)&eh->d_addr,
               (unsigned char *)&eh->s_addr, 6);
    rte_memcpy((unsigned char *)&eh->s_addr,
